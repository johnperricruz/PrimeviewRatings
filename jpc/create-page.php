								...
								<tr>
									...
									<td>
										...
										<div class="row-actions">
											<span class="view">
												<a href="#TB_inline?width=600&height=650&inlineId=rating-view" class="thickbox view-review "   data-reviewid="'.$row['ReviewID'].'" data-reviewerid="'.$row['ReviewerID'].'" data-state="'.$row['State'].'" data-city="'.$row['City'].'" data-review="'.$row['Review'].'" data-summary="'.$row['ReviewSummary'].'" data-experience="'.$row['TotalExperience'].'" data-recommend="'.$row['WillRecommend'].'" data-service="'.$row['Service'].'" data-fname="'.$row['FirstName'].'" data-lname="'.$row['LastName'].'" class="view-review"  >View</a> |  
											</span>
											<span class="restore">
						...
						while($row = $feedback->fetch_assoc()){
							...
							echo '
								<tr>
									...
									<td>'.date('M d, Y',strtotime($row['DateAnswered'])).'</td>
		...
		<?php
			echo '<table>
				<tr>
					...
					<td><input placeholder="Google Review" type="text" name="google_review" value="'. esc_attr( get_option('google_review') ).'" /></td>
				</tr>
				<tr>
					<td>Yelp Review Link</td>
					<td><input placeholder="Yelp Review" type="text" name="yelp_review" value="'. esc_attr( get_option('yelp_review') ).'" /></td>
				</tr>
				<tr>
					<td>Angie\'s List Review Link</td>
					<td><input placeholder="Angie\'s List Review" type="text" name="angies_review" value="'. esc_attr( get_option('angies_review') ).'" /></td>
				</tr>
				<tr>
					<td>Facebook Review Link</td>
					<td><input placeholder="Facebook Review" type="text" name="facebook_review" value="'. esc_attr( get_option('facebook_review') ).'" /></td>
				</tr>
			</table>';
		?>
...
function pv_save_settings_post() {
	register_setting( 'pv-option-group', 'google_review' );
	register_setting( 'pv-option-group', 'yelp_review' );
	register_setting( 'pv-option-group', 'angies_review' );
	register_setting( 'pv-option-group', 'facebook_review' );
}
